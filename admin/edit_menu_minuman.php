...
if (isset($_GET['id_menu'])) {
    ...
    $query = "SELECT * FROM menu_minuman WHERE id_menu_minuman = $id_menu";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $newMakanan = $_POST['minuman'];
    $newHarga = $_POST['harga'];
    $newStok = $_POST['sisa_stok'];

    // Buat query untuk mengupdate data menu makanan
    $updateQuery = "UPDATE menu_minuman SET minuman = '$newMakanan', harga = $newHarga, sisa_stok = $newStok WHERE id_menu_minuman = $id_menu";

    if ($koneksi->query($updateQuery) === TRUE) {
header("Location: menu_minuman.php?success=Menu+makanan+berhasil+diperbarui");
exit();
    ...
    <div class="card p-5 mb-5">
      <form method="POST" >
        <div class="form-group">
          <label for="minuman">Nama Menu</label>
          <input type="text" class="form-control" id="minuman" name="minuman" value="<?php echo $row['minuman']; ?>" required>
        </div>
        <div class="form-group">
